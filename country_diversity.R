...
if (!exists("indices")) {
  indices <- read.csv("indices.csv", check.names = FALSE)
  indices <- indices[, !names(indices) %in% c("", "X"), drop = FALSE]
}

indices <- indices %>%
  mutate(year0 = as.integer(str_sub(year_category, 1, 4))) %>%
  rename(any_of(c(country = "Site 1: Country"))) %>%
  rename(any_of(c(country = "Site.1..Country")))

...
migration_RMSD             <- migration_RMSD %>%
  mutate(std_total_flow = as.numeric(scale(total_flow)))
...
# specification.
#
# Quasi-Poisson is used throughout. RMSD is a bounded, non-negative
# continuous outcome, not a count, so the justification is the log link
# and the non-negative support rather than the count likelihood - the
# same pseudo-likelihood argument used for PPML in gravity models. The
# dispersion parameter below is well BELOW 1, i.e. the data are
# under-dispersed relative to Poisson; the quasi likelihood estimates
# that dispersion rather than fixing it at 1.
# =====================================================================
...
# dispersion parameter: 1 would be Poisson, < 1 is under-dispersion
...
# QAIC, the quasi-likelihood analogue of AIC: deviance / phi + 2k
qaic <- function(m, dispersion) {
  deviance(m) / dispersion + 2 * length(coef(m))
}

print(qaic(quasi_pois, phi))


# =====================================================================
# 4. Lag or no lag
#
# Compares fit with and without a one-period lag of total flow.
#
# lag_total_flow is NA in each pair's first period, so the lagged model
# would otherwise be fitted to ~9,200 fewer rows. Information criteria
# are not comparable across different samples, so BOTH models are fitted
# to the rows where the lag exists.
# =====================================================================
lag_data <- migration_adj_RMSD_absdiff %>% filter(!is.na(lag_total_flow))
message("lag comparison sample: ", nrow(lag_data), " of ",
        nrow(migration_adj_RMSD_absdiff), " rows")

nolag_model <- fixest::feglm(RMSD ~ total_flow + abs_diff_prev | year0 + big_region_pairs,
                             data = lag_data,
                             family = quasipoisson(link = "log"))
...
# common dispersion from the larger model, so QAIC is comparable
phi_common <- deviance(lagged_model) / df.residual(lagged_model)

qaic_nolag <- qaic(nolag_model,  phi_common)
qaic_lag   <- qaic(lagged_model, phi_common)

print(c(no_lag = qaic_nolag, with_lag = qaic_lag))
...
# fixest objects carry no null.deviance, so fit the intercept-only model
# on the same data and fixed effects and compare deviances.
null_17 <- fixest::feglm(RMSD ~ 1 | year0,
                         data = migration_adj_RMSD_absdiff,
                         fixef.rm = 'infinite_coef',
                         family = quasipoisson(link = "log"))

pseudo_r2  <- 1 - deviance(model_17) / deviance(null_17)
